...
import { createVoteEvent, parseVoteEvent, obfuscateId, buildDTag } from "./nostr-client"
...
describe("createVoteEvent", () => {
  it("creates a default room event with vote d-tag", () => {
    const template = createVoteEvent({
      ...
      votedIds: ["a".repeat(64), "b".repeat(64)],
    })
    ...
    expect(content.votes).toEqual(["a".repeat(64), "b".repeat(64)])
  })
  ...
  it("creates a private room event with pvote d-tag", () => {
    ...
    const content = JSON.parse(template.content)
    expect(content.votes).toEqual(["a".repeat(64)])
  })
...
describe("buildDTag", () => {
  it("builds default room d-tag", () => {
    expect(buildDTag("app", { type: "default", date: "2026-03-25" }))
      .toBe("app/vote/2026-03-25")
  })

  it("builds private room d-tag", () => {
    expect(buildDTag("app", { type: "private", roomId: "room1", date: "2026-03-25" }))
      .toBe("app/pvote/room1/2026-03-25")
  })
})
...
describe("parseVoteEvent", () => {
  it("extracts pubkey and votes from a vote event", () => {
    const event = {
      ...
      content: JSON.stringify({ votes: ["a".repeat(64), "b".repeat(64)] }),
      id: "evtid",
      sig: "sig",
    }
    ...
    expect(parsed!.votes).toEqual(["a".repeat(64), "b".repeat(64)])
  ...
  it("returns null when votes are strings but not valid hex-64 hashes", () => {
    const event = {
      kind: 30078,
      pubkey: "abc",
      created_at: 1000,
      tags: [],
      content: JSON.stringify({ votes: ["not-a-hash", "<script>alert(1)</script>"] }),
      id: "evtid",
      sig: "sig",
    }
