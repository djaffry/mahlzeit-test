...
**Discover lunch menus. Decide where to eat.**

Peckish aggregates daily lunch menus from a wide range of restaurants near Austria Campus in Vienna.

Most restaurants in the area only publish their menus in German. For colleagues who don't speak German well, that turns lunch into a guessing or lookup game. Peckish scrapes these menus, translates them automatically, and lets everyone read the menu in a language they understand.
...
The project is split into three domains that communicate through the shared `data/` directory. Each module has its own `package.json` and can be built and run independently.

```
project_root/
  data/                 # Globals: index.json, tags.json, languages.json, config.json.
...
- **Link adapters** just point to an external URL which is useful for restaurants without a rotating menu that you still want listed
...
```
ui/
  main.ts               # App bootstrap (theme toggle, service worker)
  ...
  views/                # App orchestration, keyboard setup
  components/           # Timeline, restaurant sections, menu items, search, filters, map panel,
                        #   dice, share, overlay, more-menu, sidebar TOC, theme,
                        #   shortcuts modal, header scroll, footer, empty state,
                        #   deep link, stale banner
  i18n/                 # Translation system with de.json and en.json
  styles/               # Modular CSS (tokens, reset, header, icon-colors)
  utils/                # Date, DOM, haptic, keyboard registry, scroll, device,
                        #   tag hierarchy, SVG, overlay helpers
```
...
# First-time local data bootstrap: scrape menus and translate. Populates data/de/, data/en/
...
# Full pipeline: scrape, translate, test
npm run verify       
```
...
Peckish runs entirely without a backend server. The frontend is a static site on GitHub Pages and menu data is generated by CI. There is no database, no API server, no cloud functions, no hosting bill. No account or signup is needed, not to use the app, not to host it, not to develop on it. Everything runs on GitHub's free tier (Actions + Pages).
...
| `localStorage` | Yes | Filter state, collapsed cards |
